...
class Trainer():
    ...
    #     self.ap = nn.AvgPool2d(4, stride=4)
    #     self.up = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=False)
    #
    # def wave_detect(self, img):
    #     img_ = self.mp(img) - self.ap(img)
    #     return self.up(img_)
    ...
    def train(self):
        ...
        for batch, (lr, edge, hr, _,) in enumerate(self.loader_train):
            ...
            sr, m = self.model(lr, 0)

            loss = self.loss(sr, hr) + self.loss(m, edge)
